...
function VideoDisplay() {
  ...
  const localStreamRef = useRef<MediaStream | null>(null);
  const animationFrameRef = useRef<number | null>(null);
  const [isWsOpen, setIsWsOpen] = useState(false); // WebSocket 상태 관리 플래그
  ...
  const initWebSocket = () => {
    if (isWsOpen) return; // 이미 WebSocket이 열려 있으면 중복 연결 방지
    ...
    socket.onopen = () => {
      ...
      setErrorMessage('');
      setWs(socket);
      setIsWsOpen(true); // WebSocket이 열린 상태로 플래그 설정
    ...
    socket.onclose = (event) => {
      ...
      setWs(null);
      setIsWsOpen(false); // WebSocket이 닫힌 상태로 플래그 설정

      // WebSocket이 비정상적으로 종료된 경우에만 재연결 시도
      if (event.code !== 1000) {
        console.log('Attempting to reconnect WebSocket...');
        setTimeout(initWebSocket, 1000); // 1초 후 재연결 시도
      }
