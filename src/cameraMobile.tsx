  ...
  const captureAndDisplayImage = () => {
    if (canvasRef.current && videoRef.current) {
      ...
      if (ctx) {
        ctx.drawImage(videoRef.current, 0, 0, canvas.width, canvas.height);
        const fullImageData = canvas.toDataURL('image/png');
        setCapturedImage(fullImageData); // 이미지를 화면에 표시하기 위해 상태에 저장
        console.log('Image captured and ready to display:', fullImageData);
      }
    }
  };
        ...
        {!isStreaming ? (
          ...
          <>
            <button
              id="captureButton"
              className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600"
              onClick={captureAndDisplayImage}
            >
              Capture & Display
            </button>
            <button
